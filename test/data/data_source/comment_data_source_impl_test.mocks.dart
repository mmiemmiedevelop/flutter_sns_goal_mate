// 목파일 작성법
// 이파일은 파이어베이스 서버를 통하지 않고 자체적으로 목파일을 생성 해주는 파일입니다
// 목파일을 생성해주는 방법은 다음과 같습니다
//
// 목파일(Mock) 생성 과정 상세 설명:
// 1. pubspec.yaml에 필요한 의존성 추가 > 이건 최초에 했으므로 생략(이미있서용)
//    dev_dependencies:
//      mockito: ^5.4.6
//      build_runner: ^2.4.8
//
// 2. 테스트 파일에서 @GenerateMocks 어노테이션으로 목 클래스 지정 : 쓰고싶은 파베 클래스 모두 추가하라는 뜻
//    예: @GenerateMocks([FirebaseFirestore, CollectionReference, DocumentReference])
//
// 3. 터미널에서 목파일 생성 명령어 실행:
//    flutter packages pub run build_runner build
//    또는 테스트 코드안에 변경사항이 있을 때마다 자동 재생성:
//    flutter packages pub run build_runner watch
//
// 4. 생성된 목파일은 자동으로 .mocks.dart 확장자로 생성됩니다
//
// 5. 테스트에서 사용 방법:
//    - MockFirebaseFirestore() 등으로 목 객체 생성
//    - when().thenReturn() 패턴으로 동작 정의
//    - verify()로 메서드 호출 검증
//
// 주의사항:
// - 이 파일은 자동 생성되므로 직접 수정하지 말것
// - 목파일을 다시 생성하려면 파일을 삭제 후 build_runner 실행
// - 의존성 변경 후에는 반드시 목파일 재생성 필요

// Mocks generated by Mockito 5.4.6 from annotations
// in flutter_princess/test/data/data_source/comment_data_source_impl_test.dart.
